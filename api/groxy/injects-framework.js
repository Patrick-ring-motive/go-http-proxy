...
void async function InjectsWithFramework() {
  ...
  importScript('/sw.js');
  queryApplyAll('link[rel*="icon"]', {
    remove: []
  });
  head().appendChild(buildElement('link', {
    values: {
      href: '/images/favicon-gopher.png'
    },
    attributes: {
      rel: 'icon',
      sizes: 'any'
  ...
  head().appendChild(buildElement('link', {
    values: {
      href: '/images/favicon-gopher-plain.png'
    },
    attributes: {
  ...
  head().appendChild(buildElement('link', {
    values: {
      href: '/images/favicon-gopher.svg'
    },
    attributes: {
      rel: 'icon',
      type: 'image/svg+xml'
  ...
  if (!select('[id="injectcss"]')) {
    body().appendChild(buildElement('style', {
      values: {
        id: 'injectcss',
        innerHTML: await (await fetch(`${location.origin}/groxy/injects.css`)).text()
      }
    }));
  }
